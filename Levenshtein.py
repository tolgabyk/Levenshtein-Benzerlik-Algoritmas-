...
def LevenshteinMesafesi(A,B):
    ...
    for i in range(A_len): #satır
        K[i][0] = i
    for i in range(B_len): #sütun
        K[0][i] = i
...
print("'" + kelime_1+ "' ve '"+kelime_2+"' arasındaki Levenshtein Mesafesi :",format(mesafe)  )

benzerlik_oran = ((max_len - mesafe)/max_len)*100 #benzerlik oran yüzdesi

print(f"Benzerlik Oranı: % {benzerlik_oran}")
